...
for epoch in range(opt.num_epochs):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(val_dataloader):
            ...
            loss, loss_dict_ema = model.validation_step(batch=batch, batch_idx=batch_idx, save_config=save_config)

            if batch_idx % 10 == 0:
                val_info = '[Validation] Epoch: {}, mse_loss: {:.4f}'.format(epoch, loss.cpu().item()) + str(loss_dict_ema)
