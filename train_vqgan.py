...
for epoch in range(opt.num_epochs):
    ...
    for batch in train_dataloader:
        if batch_index == save_index:
            save_config['flag'] = False
            save_config['split'] = 'train'
            save_config['epoch'] = epoch
            save_config['global_step'] = global_step
            save_config['batch_index'] = batch_index
        else:
            save_config['flag'] = False
            save_config['split'] = 'train'
            save_config['epoch'] = epoch
    ...
    with torch.no_grad():
        ...
        for batch in val_dataloader:
            ...
            # if batch_index == save_index:
            #     log = model.log_images(batch, device)
            #     save_img(log, 'val', epoch, global_step, batch_index)
